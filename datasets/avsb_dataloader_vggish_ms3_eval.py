...
class S4Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        audio_lm_path = os.path.join(self.args.config['AVSBenchMS3']['DIR_AUDIO_LOG_MEL'], self.split,
                                     video_name + '.pkl')
        ...
        for img_id in range(1, self.mask_num + 1):
            img, img_PIL = load_image_in_PIL_to_Tensor(
                os.path.join(img_base_path, video_name, "%s.mp4_%d.png" % (video_name, img_id)),
        ...
        for mask_id in range(1, self.mask_num + 1):
            mask = load_mask_image_in_PIL_to_Tensor(os.path.join(mask_base_path, "%s_%d.png" % (video_name, mask_id)),
