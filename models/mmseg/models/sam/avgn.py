...
class AVGN(nn.Module):
    def __init__(self, tau, dim, dropout_img, dropout_aud, attn_assign, num_class, depth_aud, depth_vis):
        ...
        self.unimodal_assgin = attn_assign
        unimodal_hard_assignment = True if attn_assign == 'hard' else False

        # learnable tokens
        self.num_class = num_class
        self.av_token = nn.Parameter(torch.zeros(num_class, dim))
        ...
        self.fc_cls = nn.Linear(dim, num_class)
    ...
    def forward(self, image, audio, mode='train', cls_target=None):
        ...
        if mode == 'train':
            ...
            cls_pred_loss = self.cls_pred_loss(v_pred_prob, cls_target_v) + self.cls_pred_loss(a_pred_prob,
                                                                                               cls_target_a)
            return loss, Savl, cls_token_loss, cls_pred_loss
        elif mode == 'test':
            return Savl, xv_attn, xa_attn, xv
